...
def main():
    base_save_path = "../../harmlessness/PkuRmb"
    
    # 1. RMB Target -- download RMB data first from https://github.com/Zhou-Zoey/RMB-Reward-Model-Benchmark/tree/main/RMB_dataset/Pairwise_set/Harmlessness
    rmb_raw_path = "../raw_data/RMB-Harmlessness"
    rmb_data = load_rmb_all(rmb_raw_path)
    random.shuffle(rmb_data)
    prepare_and_save(rmb_data, os.path.join(base_save_path, "PkuRmb-Target"), is_rmb=True)
    ...
    prepare_and_save(safe_list, os.path.join(base_save_path, "PkuRmb-Bench"), is_rmb=False)

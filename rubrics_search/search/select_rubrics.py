# select topk rubrics from each generation groups, respectively
...
for i in range(ITERATIONS):
    ...
    df = df.sort_values(by=["fitness", "target_acc"], ascending=[False, True]).head(TOPK)
    want_ids = set(df["prompt_id"].astype(str).tolist())
